...
public class GreenDaoMaster extends AbstractDaoMaster {
     ...
     */
    public static GreenDaoSession newDevSession(Context context, String name) {
        Database db = new DevOpenHelper(context, name).getWritableDb();
        GreenDaoMaster daoMaster = new GreenDaoMaster(db);
        return daoMaster.newSession();
    }

    public GreenDaoMaster(Database db) {
        super(db, SCHEMA_VERSION);
        registerDaoClass(AppDao.class);
    }

    public GreenDaoSession newSession() {
        return new GreenDaoSession(db, IdentityScopeType.Session, daoConfigMap);
    }

    public GreenDaoSession newSession(IdentityScopeType type) {
        return new GreenDaoSession(db, type, daoConfigMap);
    }
