...
import com.wolcano.musicplayer.music.ui.fragments.folder.foldersearch.MaterialSearchLast;
 ...
 */
public class FragmentMain extends BaseFragment implements SetSearchQuery, AdapterClickListener, InterstitialListener {
...
>>>>>>> backup:app/src/main/java/com/wolcano/musicplayer/music/ui/fragments/FragmentOnline.java
    ...
    private void setSearchView() {
        ...
        int coloro = Utils.getPrimaryColor(context);
        int colorTint;
        if (Utils.isColorLight(coloro)) {
        ...
        materialSearchView.setBackgroundColor(coloro);
        ColorDrawable sdasd = new ColorDrawable(coloro);
        materialSearchView.setSuggestionBackground(sdasd);
        ...
        materialSearchView.setOnQueryTextListener(new MaterialSearchLast.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                ...
                materialSearchView.setSuggestions(arrf, arrf2, false, FragmentMain.this, colorTint);
                ...
                if (Utils.isItTrue(context) && !query.trim().contains(val)) {
                    setSearchQuery(query.trim());
                } else if (!Utils.isItTrue(context) || query.trim().contains(val)) {
            ...
            public boolean onQueryTextChange(String newText) {
                if (newText.equals("")) {
                    ...
                    if (str != null) {
                        ...
                        materialSearchView.setSuggestions(arr, lastSearches, true, FragmentMain.this, colorTint);
                    }
                    ...
                    isitInitial = false;
                    materialSearchView.setSuggestions(suggestionListStringsFromRemove, lastSearchesStringsFromRemove, false, FragmentMain.this, colorTint);
    ...
    private void handleAutoSearch() {
        ...
        if (getAuto) {
            ...
            if (!last_single_search.isEmpty() && Utils.isItTrue(context) && !last_single_search.trim().contains(val)) {
                setSearchQuery(last_single_search.trim());
            } else if (!Utils.isItTrue(context) || last_single_search.trim().contains(val)) {
    ...
    public void setSearchQuery(String query) {
        ...
        if (!lMore) {
            ...
            lCounter = 0;
            srcCnt++;
            if (srcCnt % 2 == 1 && srcCnt != 2 || srcCnt == 1) {
                showInterstitial();
            }
            posMore = 0;
            if (mAdapter != null) {
    ...
    private void setMopubAdapter() {
        mAdapter = new MainAdapter((AppCompatActivity) getActivity(), arrayModel1List, this, this);
        myMoPubAdapter = new MoPubRecyclerAdapter(getActivity(), mAdapter);
