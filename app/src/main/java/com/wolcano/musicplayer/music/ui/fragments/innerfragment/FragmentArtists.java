...
public class FragmentArtists extends BaseFragment implements AdapterClickListener, AppBarLayout.OnOffsetChangedListener {
    ...
    private void setMopubAdapter(List<Artist> artistList) {
        if (artistList.size() <= 30) {
            recyclerView.setThumbEnabled(false);
        } else {
            recyclerView.setThumbEnabled(true);
        }
        adapter = new ArtistAdapter((MainActivity) getActivity(), artistList, FragmentArtists.this);
        controlIfEmpty();
